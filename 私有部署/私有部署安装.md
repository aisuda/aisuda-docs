...
推荐使用 Linux 环境，最小机器配置建议是 4 核 4G 内存。
...
services:
  web:
    image: registry.baidubce.com/aisuda/aisuda:1.1.10
    restart: always
    ports:
      - '8090:8085'
    environment:
...
```
docker pull registry.baidubce.com/aisuda/aisuda:1.1.10
docker save -o suda.tar registry.baidubce.com/aisuda/aisuda:1.1.10
...
```
# 下载 aisuda 最新版本
docker run --rm -it --name aisuda registry.baidubce.com/aisuda/aisuda:1.1.10 bash
