...
services:
  web:
    image: registry.baidubce.com/aisuda/aisuda:1.1.0
    restart: always
    ports:
      - '8090:8085'
    environment:
...
进入 MySQL 容器后使用 `mysql -uroot -p'Q39sTi0i^9'` 命令访问 MySQL，注意密码改成对应的，然后使用 `CREATE DATABASE aisuda;` 命令来创建数据库。
...
```
docker pull registry.baidubce.com/aisuda/aisuda:1.1.0
docker save -o suda.tar registry.baidubce.com/aisuda/aisuda:1.1.0
